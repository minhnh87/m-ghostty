...
/// Programmatically generates a green "H" app icon using Core Graphics.
struct HGreenAppIcon {
    static let size = CGSize(width: 1024, height: 1024)

    /// Generate the green "H" icon as an NSImage.
    ...
    /// Draw the bold white "H" with a drop shadow.
    private static func drawLetter(in context: CGContext, rect: CGRect) {
        ...
        let attrString = NSAttributedString(string: "H", attributes: attributes)
