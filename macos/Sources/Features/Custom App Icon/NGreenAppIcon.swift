...
/// Programmatically generates a green "N" app icon using Core Graphics.
struct NGreenAppIcon {
    static let size = CGSize(width: 1024, height: 1024)

    /// Generate the green "N" icon as an NSImage.
    ...
    /// Draw the bold white "N" with a drop shadow.
    private static func drawLetter(in context: CGContext, rect: CGRect) {
        ...
        let attrString = NSAttributedString(string: "N", attributes: attributes)
