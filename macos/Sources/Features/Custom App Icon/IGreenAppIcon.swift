...
/// Programmatically generates a green "I" app icon using Core Graphics.
struct IGreenAppIcon {
    static let size = CGSize(width: 1024, height: 1024)

    /// Generate the green "I" icon as an NSImage.
    static func makeImage() -> NSImage? {
        ...
        // -- Letter "I" with shadow --
        drawLetter(in: context, rect: iconRect)
    ...
    /// Draw the bold white "I" with a drop shadow.
    private static func drawLetter(in context: CGContext, rect: CGRect) {
        context.saveGState()
        defer { context.restoreGState() }
        ...
        let attrString = NSAttributedString(string: "I", attributes: attributes)
